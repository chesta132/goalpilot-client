    ...
    <div className={`relative ${className}`}>
      ...
      <label
        className={`
          ...
          ${isFocus ? "-top-2.5 left-3 text-xs text-(--accent) font-medium bg-(--theme) px-1" : "top-3.5 left-3 text-sm text-(--gray)"}
        `}
        htmlFor={label}
      >
        {isFocus ? toCapitalize(label || "") || placeholder : placeholder} {isFocus && optional && <span className="text-(--gray)">(Optional)</span>}
      </label>
      {password && (
      ...
      {error && <p className="text-red-500 text-[12px] text-start">{error}</p>}
    </div>
  );
};
